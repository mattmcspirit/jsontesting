...
{ 
   ...
   ( 
    ...
    Node localhost
    {
        ...
        xDnsServerPrimaryZone "Set Primary Zone"
        {
            Name = 'akshci.local'
            Ensure = 'Present'
            DependsOn = "[script]NAT rule"
            ZoneFile: 'akshci.local.dns'
            DynamicUpdate: 'NonSecureAndSecure'
        }

        xDnsServerPrimaryZone "Set Reverse Lookup Zone"
        {
            Name = '0.168.192.in-addr.arpa'
            Ensure = 'Present'
            DependsOn = "[xDnsServerPrimaryZone]Set Primary Zone"
            ZoneFile: '0.168.192.in-addr.arpa.dns'
            DynamicUpdate: 'NonSecureAndSecure'
        }
            
        xDnsServerForwarder "Set Forwarder"
        {
            IsSingleInstance = 'Yes'
            IPAddresses = 'Present'
            DependsOn = "[xDnsServerPrimaryZone]Set Reverse Lookup Zone"
        }

        xDnsServerSetting "Set Listener"
        {
            ListenAddresses: = '1.1.1.1','1.0.0.1'
            DependsOn = "[xDnsServerForwarder]Set Forwarder"
        }
        ...
        Script SetDHCPDNSSetting
	    {
            ...
            GetScript =  { @{} }
            TestScript = { $false }
	        DependsOn = "[xDnsServerSetting] Set Listener"
        }
